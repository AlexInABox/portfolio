...
function reorderElements() {
  ...
  cardsArray.sort(function (a, b) {
    return (
      parseInt(b.getAttribute("timestamp")) -
      parseInt(a.getAttribute("timestamp"))
    );
  });
...
async function patchLiveContent() {
  ...
  document
    ...
    .setAttribute("timestamp", data.health.lastUpdate.heartRate);
  ...
  document
    ...
    .setAttribute("timestamp", data.health.lastUpdate.speed);
}

async function patchRegularContent() {
  if (
    data.location.district !== localData.district ||
    data.location.city !== localData.city
  ) {
  ...
  if (
    data.valorant.username !==
      document.getElementById("valorant.username").innerHTML ||
    data.valorant.rank + " - " + data.valorant.rr + " RR" !==
      document.getElementById("valorant.rank").innerHTML
  ) {
    getLatestValorantInformation();
    console.log("valorant changed");
  }
  if (
    data.youtube.video.url !== document.getElementById("youtubeVideoURL").href
  ) {
    getLatestYouTubeVideoInformation();
    console.log("youtube video changed");
  }
  if (
    data.youtube.music.url !== document.getElementById("youtubeMusicURL").href
  ) {
    getLatestYouTubeMusicInformation();
...
function updateOxygenSaturationBar(value) {
  ...
  function frame() {
    if (width > value) {
      ...
      document.getElementById("oxygenSaturation-bar").innerHTML =
        elem.style.width;
    } else if (width < value) {
      width++;
      elem.style.width = width + "%";
  ...
  document
    ...
    .setAttribute("timestamp", data.health.lastUpdate.oxygenSaturation);
}
...
function getLatestPositionSnapshot() {
  ...
  document.getElementById("mapbox").src =
    `https://api.mapbox.com/styles/v1/alexinabox/clgw8e3rx003j01qth2z003ms/draft/static/${data.location.longitude},${data.location.latitude},11.3,0,35/500x500?access_token=pk.eyJ1IjoiYWxleGluYWJveCIsImEiOiJjbGd3ODh2YmswOTd1M2hwZ2RyY3E1Nnh6In0.YT0f1GOC9fGnLpS67CAOIw`;

  if (data.location.district) {
    //check if the district is defined or not null (empty string)
    document.getElementById("locationText").innerHTML =
      `📍 ${data.location.district}, ${data.location.country}`;
  } else {
    document.getElementById("locationText").innerHTML =
  ...
  document
    ...
    .setAttribute("timestamp", data.location.lastUpdate);
}
...
function getLatestNetflixInformation() {
  document.getElementById("netflixShowName").innerHTML =
    `🎬 ${data.netflix.lastWatched.title}`;
  document.getElementById("netflixShowName").href =
    `https://www.netflix.com/title/${data.netflix.lastWatched.showId}`;
  document.getElementById("netflixCover").src =
    data.netflix.lastWatched.defaultImage;
  document.getElementById("netflixCoverLink").href =
    `https://www.netflix.com/title/${data.netflix.lastWatched.showId}`;
  document
    .getElementById("netflix")
    .setAttribute("timestamp", data.netflix.lastWatched.lastUpdate);
}

function getLatestPlexInformation() {
  document.getElementById("plexShowName").innerHTML =
    `🎬 ${data.plex.lastWatched.title}`;
  document.getElementById("plexCover").src =
    "./custom-hds/plex_covers/" + data.plex.lastWatched.cover;
  document.getElementById("plexCoverLink").href =
    data.plex.lastWatched.publicURL;
  document
    .getElementById("plex")
    .setAttribute("timestamp", data.plex.lastWatched.lastUpdate);
}
...
async function getLatestValorantInformation() {
  ...
  document.getElementById("valorant.rank").innerHTML =
    `${data.valorant.rank} - ${data.valorant.rr} RR`;
  document
    .getElementById("valorant.rankIcon")
    .setAttribute("xlink:href", await fetchRankIcon(data.valorant.rank));
  document.getElementById("valorant.mmrMeter").style.stroke = getColorForRank(
    data.valorant.rank,
  );
  document
    .getElementById("valorant")
    .setAttribute("timestamp", data.valorant.lastUpdate);
}

function getLatestDuolingoInformation() {
  document.getElementById("duolingoUsername").innerHTML =
    data.duolingo.username;
  document.getElementById("duolingoAvatar").src = data.duolingo.avatar;

  document.getElementById("duolingoLanguageFlag").src =
    data.duolingo.language_icon_URL;

  document.getElementById("duolingoStreakXP").innerHTML =
    `${data.duolingo.streak}🔥 - ${data.duolingo.xp} XP`;
  document
    .getElementById("duolingo")
    .setAttribute("timestamp", data.duolingo.lastUpdate);
}
  ...
  return new Promise((obj) => {
    fetch("https://valorant-api.com/v1/competitivetiers")
      ...
      .then((data) => {
        ...
        for (
          var i = 0;
          i < rankList.data[rankList.data.length - 1].tiers.length;
          i++
        ) {
          if (
            rankList.data[rankList.data.length - 1].tiers[
              i
            ].tierName.toLowerCase() === rank.toLowerCase()
          ) {
            return obj(
              rankList.data[rankList.data.length - 1].tiers[i].largeIcon,
            );
          }
        }
      });
...
function setLastUpdatedTime() {
  ...
  if (locationDiffSeconds < 60) {
    document.getElementById("locationLastUpdated").innerHTML =
      `${locationDiffSeconds} seconds ago`;
  } else if (locationDiffSeconds < 3600) {
    document.getElementById("locationLastUpdated").innerHTML =
      `${Math.floor(locationDiffSeconds / 60)} minutes ago`;
  } else if (locationDiffSeconds < 86400) {
    document.getElementById("locationLastUpdated").innerHTML =
      `${Math.floor(locationDiffSeconds / 3600)} hours ago`;
  } else {
    document.getElementById("locationLastUpdated").innerHTML =
      `${Math.floor(locationDiffSeconds / 86400)} days ago`;
  }

  if (netflixDiffSeconds < 60) {
    document.getElementById("netflixLastUpdated").innerHTML =
      `${netflixDiffSeconds} seconds ago`;
  } else if (netflixDiffSeconds < 3600) {
    document.getElementById("netflixLastUpdated").innerHTML =
      `${Math.floor(netflixDiffSeconds / 60)} minutes ago`;
  } else if (netflixDiffSeconds < 86400) {
    document.getElementById("netflixLastUpdated").innerHTML =
      `${Math.floor(netflixDiffSeconds / 3600)} hours ago`;
  } else {
    document.getElementById("netflixLastUpdated").innerHTML =
      `${Math.floor(netflixDiffSeconds / 86400)} days ago`;
  }

  if (plexDiffSeconds < 60) {
    document.getElementById("plexLastUpdated").innerHTML =
      `${plexDiffSeconds} seconds ago`;
  } else if (plexDiffSeconds < 3600) {
    document.getElementById("plexLastUpdated").innerHTML =
      `${Math.floor(plexDiffSeconds / 60)} minutes ago`;
  } else if (plexDiffSeconds < 86400) {
    document.getElementById("plexLastUpdated").innerHTML =
      `${Math.floor(plexDiffSeconds / 3600)} hours ago`;
  } else {
    document.getElementById("plexLastUpdated").innerHTML =
      `${Math.floor(plexDiffSeconds / 86400)} days ago`;
  }

  if (valorantDiffSeconds < 60) {
    document.getElementById("valorantLastUpdated").innerHTML =
      `${valorantDiffSeconds} seconds ago`;
  } else if (valorantDiffSeconds < 3600) {
    document.getElementById("valorantLastUpdated").innerHTML =
      `${Math.floor(valorantDiffSeconds / 60)} minutes ago`;
  } else if (valorantDiffSeconds < 86400) {
    document.getElementById("valorantLastUpdated").innerHTML =
      `${Math.floor(valorantDiffSeconds / 3600)} hours ago`;
  } else {
    document.getElementById("valorantLastUpdated").innerHTML =
      `${Math.floor(valorantDiffSeconds / 86400)} days ago`;
  }

  if (youtubeVideoDiffSeconds < 60) {
    document.getElementById("youtubeVideoLastUpdated").innerHTML =
      `${youtubeVideoDiffSeconds} seconds ago`;
  } else if (youtubeVideoDiffSeconds < 3600) {
    document.getElementById("youtubeVideoLastUpdated").innerHTML =
      `${Math.floor(youtubeVideoDiffSeconds / 60)} minutes ago`;
  } else if (youtubeVideoDiffSeconds < 86400) {
    document.getElementById("youtubeVideoLastUpdated").innerHTML =
      `${Math.floor(youtubeVideoDiffSeconds / 3600)} hours ago`;
  } else {
    document.getElementById("youtubeVideoLastUpdated").innerHTML =
      `${Math.floor(youtubeVideoDiffSeconds / 86400)} days ago`;
  }

  if (youtubeMusicDiffSeconds < 60) {
    document.getElementById("youtubeMusicLastUpdated").innerHTML =
      `${youtubeMusicDiffSeconds} seconds ago`;
  } else if (youtubeMusicDiffSeconds < 3600) {
    document.getElementById("youtubeMusicLastUpdated").innerHTML =
      `${Math.floor(youtubeMusicDiffSeconds / 60)} minutes ago`;
  } else if (youtubeMusicDiffSeconds < 86400) {
    document.getElementById("youtubeMusicLastUpdated").innerHTML =
      `${Math.floor(youtubeMusicDiffSeconds / 3600)} hours ago`;
  } else {
    document.getElementById("youtubeMusicLastUpdated").innerHTML =
      `${Math.floor(youtubeMusicDiffSeconds / 86400)} days ago`;
  }

  if (duolingoDiffSeconds < 60) {
    document.getElementById("duolingoLastUpdated").innerHTML =
      `${duolingoDiffSeconds} seconds ago`;
  } else if (duolingoDiffSeconds < 3600) {
    document.getElementById("duolingoLastUpdated").innerHTML =
      `${Math.floor(duolingoDiffSeconds / 60)} minutes ago`;
  } else if (duolingoDiffSeconds < 86400) {
    document.getElementById("duolingoLastUpdated").innerHTML =
      `${Math.floor(duolingoDiffSeconds / 3600)} hours ago`;
  } else {
    document.getElementById("duolingoLastUpdated").innerHTML =
      `${Math.floor(duolingoDiffSeconds / 86400)} days ago`;
  }
}
...
function setLastUpdateTimeForLiveData() {
  ...
  if (heartRateDiffSeconds < 60) {
    document.getElementById("heartRateLastUpdated").innerHTML =
      `${heartRateDiffSeconds} seconds ago`;
  } else if (heartRateDiffSeconds < 3600) {
    document.getElementById("heartRateLastUpdated").innerHTML =
      `${Math.floor(heartRateDiffSeconds / 60)} minutes ago`;
  } else if (heartRateDiffSeconds < 86400) {
    document.getElementById("heartRateLastUpdated").innerHTML =
      `${Math.floor(heartRateDiffSeconds / 3600)} hours ago`;
  } else {
    document.getElementById("heartRateLastUpdated").innerHTML =
      `${Math.floor(heartRateDiffSeconds / 86400)} days ago`;
  }

  if (oxygenSaturationDiffSeconds < 60) {
    document.getElementById("oxygenSaturationLastUpdated").innerHTML =
      `${oxygenSaturationDiffSeconds} seconds ago`;
  } else if (oxygenSaturationDiffSeconds < 3600) {
    document.getElementById("oxygenSaturationLastUpdated").innerHTML =
      `${Math.floor(oxygenSaturationDiffSeconds / 60)} minutes ago`;
  } else if (oxygenSaturationDiffSeconds < 86400) {
    document.getElementById("oxygenSaturationLastUpdated").innerHTML =
      `${Math.floor(oxygenSaturationDiffSeconds / 3600)} hours ago`;
  } else {
    document.getElementById("oxygenSaturationLastUpdated").innerHTML =
      `${Math.floor(oxygenSaturationDiffSeconds / 86400)} days ago`;
  }

  if (speedDiffSeconds < 60) {
    document.getElementById("speedLastUpdated").innerHTML =
      `${speedDiffSeconds} seconds ago`;
  } else if (speedDiffSeconds < 3600) {
    document.getElementById("speedLastUpdated").innerHTML =
      `${Math.floor(speedDiffSeconds / 60)} minutes ago`;
  } else if (speedDiffSeconds < 86400) {
    document.getElementById("speedLastUpdated").innerHTML =
      `${Math.floor(speedDiffSeconds / 3600)} hours ago`;
  } else {
    document.getElementById("speedLastUpdated").innerHTML =
      `${Math.floor(speedDiffSeconds / 86400)} days ago`;
  }
}

async function getLatestYouTubeVideoInformation() {
  document.getElementById("youtubeVideoTitle").innerHTML =
    `${data.youtube.video.title}`;
  if (withOrWithoutBy === 1) {
    document.getElementById("youtubeVideoChannel").innerHTML =
      `by: ${data.youtube.video.channel}`;
  } else {
    document.getElementById("youtubeVideoChannel").innerHTML =
      `${data.youtube.video.channel}`;
  }
  document.getElementById("youtubeVideoURL").href = data.youtube.video.url;
  document.getElementById("youtubeVideoThumbnail").src =
    data.youtube.video.thumbnail;

  document
    .getElementById("youtube video")
    .setAttribute("timestamp", data.youtube.video.lastUpdate);
}

async function getLatestYouTubeMusicInformation() {
  document.getElementById("youtubeMusicTitle").innerHTML =
    `${data.youtube.music.title}`;
  if (withOrWithoutBy === 1) {
    document.getElementById("youtubeMusicArtist").innerHTML =
      `by: ${data.youtube.music.artist}`;
  } else {
    document.getElementById("youtubeMusicArtist").innerHTML =
      `${data.youtube.music.artist}`;
  }
  document.getElementById("youtubeMusicURL").href = data.youtube.music.url;
  document.getElementById("youtubeMusicThumbnail").src =
    data.youtube.music.thumbnail;

  document
    .getElementById("youtube music")
    .setAttribute("timestamp", data.youtube.music.lastUpdate);
}
